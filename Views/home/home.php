            ...
            if (!empty($transacs)) :
              foreach ($transacs as $transac) :
                ...
                if ($income->user_id && $transac->user_id == $_SESSION['user']['id']) : ?>
                  <!-- sinon on set à 0 -->
                  <h2 class="text-white"> <?= Utils::numberFormat($income->total_amount - $transac->total_amount); ?> €</h2>
        ...
        <?php if (!empty($incomes)) :
          foreach ($incomes as $income) :
            if ($income->user_id == $_SESSION['user']['id']) : ?>
              <h2 class="text-dark"><?= Utils::numberFormat($income->total_amount) ?>€</h2>
        ...
        <?php if (!empty($transacs)) :
          foreach ($transacs as $transac) :
            if ($transac->user_id == $_SESSION['user']['id']) : ?>
              <h2 class="text-dark"><?= Utils::numberFormat($transac->total_amount) ?>€</h2>
        ...
        <?php if (!empty($savings)) :
          foreach ($savings as $saving) :
            if ($saving->user_id == $_SESSION['user']['id']) : ?>
              <h2 class="text-white"><?= Utils::numberFormat($saving->total_amount) ?>€</h2>
            ...
            <table class=" table">
              ...
              <tbody>
                ...
                ?>
                    ...
                    <tr class="table-light">
                      <td><?= $trc->title; ?></td>
                      <td><?= Utils::numberFormat($trc->amount / $trc->payment_option); ?>€</td>
                      <td><?= $trc->location; ?></td>
